...
class EventsParser(FileParser):
    ...
    def __init__(self, fname, persons, families, locations):
    ...
    def model(self, r):
        refs = [self.persons, self.families]
        # TODO create function to check reuse
        if r['ref_id'] == None:  # skip reuse events
            return

        loc_id = r['loc_id']
        loc = self.locations.get(loc_id)
        if loc_id != None and not loc:
            print(f'WARNING: location not found (#{loc_id})')
        name = r['custom_name']
        # TODO create function to get event type
        ref = refs[r['ref_type']][r['ref_id']]
        event = Event(name if name else event_type_map[r['type']], date(
            r), ref, r['prepos'], loc)
